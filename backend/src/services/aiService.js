...
async function callGemini(prompt, isJsonResponse = true) {
  ...
  while (attempt < 2) {
    try {
      ...
      console.warn(`⚠️ Gemini API call failed (attempt ${attempt}). Retrying... Error:`, error.message);
 ...
 */
export async function generateInterviewQuestions({ role, difficulty, experience, technologies }) {
